...
conf.matrix <- table(eval_Data$loan, prediction)
conf.matrix


accuracy <- sum(diag(conf.matrix)) / sum(conf.matrix)
accuracy
Recall = sum(conf.matrix[2,2])/sum(conf.matrix[2,])
Recall
Precision = sum(conf.matrix[2,2])/sum(conf.matrix[,2])
Precision

xgb.dump(model, "dump.raw.txt", with_stats = T)
